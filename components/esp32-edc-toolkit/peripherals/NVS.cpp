...
bool NVS::m_IsInitialized = false;
...
esp_err_t NVS::init(){
	...
	//is there a nvs_key partition?
	if(nvs_part->encrypted){
		#ifndef CONFIG_NVS_ENCRYPTION
			ESP_LOGE(LOG_TAG, "Flash encryption failed! Use CONFIG_NVS_ENCRYPTION!");
			assert(false);
		#endif
		#ifdef CONFIG_NVS_ENCRYPTION
		nvs_sec_cfg_t cfg = nvs_sec_cfg_t();
		//is there a key pair?
		errRc = ::nvs_flash_read_security_cfg(key_part, &cfg);
		if(errRc != ESP_OK){
			//erase flash to deleat dead datablocks
			errRc = ::nvs_flash_erase();
			if (errRc != ESP_OK) {
				ESP_LOGE(LOG_TAG, "nvs_flash_erase: rc=%d %s", errRc, GeneralUtils::errorToString(errRc));
			}

			//generate the key pair now
			errRc = ::nvs_flash_generate_keys(key_part, &cfg);
			if (errRc != ESP_OK) {
				ESP_LOGE(LOG_TAG, "nvs_flash_generate_keys: rc=%d %s", errRc, GeneralUtils::errorToString(errRc));
				return errRc;
			}
		}
		// pre-init done - now, init nvs partition!
		errRc = ::nvs_flash_secure_init(&cfg);   // Initialize flash
		if (errRc == ESP_ERR_NVS_NO_FREE_PAGES || errRc == ESP_ERR_NVS_NEW_VERSION_FOUND) {
			ESP_LOGE(LOG_TAG, "nvs_flash_secure_init: rc=%d %s", errRc, GeneralUtils::errorToString(errRc));
