 ...
 * @return N/A.
 */
esp_err_t I2C::beginTransaction() {
	if (debug) {
 ...
 * @return N/A.
 */
esp_err_t I2C::init(uint8_t address, gpio_num_t sdaPin, gpio_num_t sclPin, uint32_t clockSpeed, i2c_port_t portNum) {
 ...
 * @return N/A.
 */
esp_err_t I2C::read(uint8_t* bytes, size_t length, i2c_ack_type_t ack) {
	esp_err_t errRc = ESP_FAIL;
	...
	if (m_directionKnown == false) {
		...
		errRc = ::i2c_master_write_byte(m_cmd, (m_address << 1) | I2C_MASTER_READ, ack);
		if (errRc != ESP_OK) {
	...
	errRc = ::i2c_master_read(m_cmd, bytes, length, ack);
	if (errRc != ESP_OK) {
 ...
 * @return N/A.
 */
esp_err_t I2C::read(uint8_t *byte, i2c_ack_type_t ack) {
	esp_err_t errRc = ESP_FAIL;
	if (debug) {
 ...
 * @return N/A.
 */
esp_err_t I2C::write(uint8_t byte, i2c_ack_type_t ack) {
	esp_err_t errRc = ESP_FAIL;
	...
	if (m_directionKnown == false) {
		m_directionKnown = true;
		errRc = ::i2c_master_write_byte(m_cmd, (m_address << 1) | I2C_MASTER_WRITE, ack);
		if (errRc != ESP_OK) {
			ESP_LOGE(LOG_TAG, "i2c_master_write_byte: rc=%d %s", errRc, GeneralUtils::errorToString(errRc));
			return errRc;
		}
	}
	errRc = ::i2c_master_write_byte(m_cmd, byte, ack);
	if (errRc != ESP_OK) {
		ESP_LOGE(LOG_TAG, "i2c_master_write_byte: rc=%d %s", errRc, GeneralUtils::errorToString(errRc));
	}
 ...
 * @return N/A.
 */
esp_err_t I2C::write(uint8_t *bytes, size_t length, i2c_ack_type_t ack) {
	esp_err_t errRc = ESP_FAIL;
	...
	errRc = ::i2c_master_write(m_cmd, bytes, length, ack);
	if (errRc != ESP_OK) {
