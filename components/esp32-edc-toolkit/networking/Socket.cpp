...
Socket::~Socket() {
	//close(); // When the class instance has ended, delete the socket.
}
...
int Socket::bind(uint16_t port, uint32_t address) {
	...
	//ESP_LOGD(LOG_TAG, "<< bind: port=%d, SocketFD: %d", port, m_sock);
	return rc;
} // bind
 ...
 */
int Socket::receive(uint8_t* data, size_t length, bool exact) {
...
void Socket::setSSL(bool sslValue) {
  const char* pers = "ssl_server";
	...
	if (sslValue == true) {
		...
		// #############  READ ME! #######################
		// @NOTE: Diese Funktion wurde erstmal abgeschalten.
		// Die SSL Klasse ist nichtmehr static, daher muss dies hier angepasst werden!
		// #############  READ ME! #######################


		char* pvtKey      = nullptr;//SSLUtils::getKey();
		char* certificate = nullptr;//SSLUtils::getCertificate();
		if (pvtKey == nullptr) {
